...
export default function ShortsEditorPage() {
  ...
  useEffect(() => {
    setMounted(true);
    // Auto-fetch the next short number
    const fetchNextShortNumber = async () => {
      try {
        const response = await fetch("/api/shorts/next-number");
        if (response.ok) {
          const { nextNumber } = await response.json();
          setShortNumber(nextNumber);
        } else {
          console.error("Failed to fetch next short number");
          setShortNumber("001"); // fallback
        }
      } catch (error) {
        console.error("Error fetching next short number:", error);
        setShortNumber("001"); // fallback
      }
    };

    fetchNextShortNumber();
  }, []);
...
`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const markdownFileName = `${shortNumber}-${formattedTitle}.md`;
      const coverImageFileName = `${shortNumber}-${formattedTitle}.png`;
  ...
  return (
    <div className="py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-bold mb-8 text-base-content">Create New Short</h1>

        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Short Number and Title */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="shortNumber" className="label">
                <span className="label-text">Short Number</span>
              </label>
              <input
                type="text"
                id="shortNumber"
                value={shortNumber}
                onChange={(e) => setShortNumber(e.target.value)}
            ...
            <div>
              ...
              {title && (
                <div className="mt-2 text-sm text-base-content/70">
                  <span className="font-medium">Filename preview:</span> {shortNumber}-{formatFileName(title)}.md
                </div>
              )}
            </div>
          </div>
              ...
              {isDragOver && (
                <div className="absolute inset-0 bg-primary/10 border-2 border-dashed border-primary z-10 flex items-center justify-center rounded-lg">
                  <div className="text-center">
                    ...
                    <p className="text-primary font-medium">Drop images here to add them to your short</p>
                  </div>
                </div>
              )}
              {mounted ? (
