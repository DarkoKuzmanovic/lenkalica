...
**Medium Priority:** 6. Add unit tests for critical functions (validation, markdown rendering) 7. Consider stricter iframe src restrictions in sanitize schema (domain allowlist) 8. Use crypto.timingSafeEqual for API key comparison 9. Optimize list endpoints to skip full markdown rendering (large refactor)

**Low Priority:** 10. Sanitize user-controlled input in console.error logs 11. Add monitoring for suspicious content ID attempts 12. Consider precomputing/caching article metadata at build time
